...
export class RefactoringService {
     ...
     */
    async analyzeCodeQuality(projectRoot, analysis) {
        const findings = [];
        ...
        for (const filePath of tsFiles) {
            try {
                const content = fs.readFileSync(filePath, 'utf8');
                const rel = path.relative(projectRoot, filePath).replace(/\\/g, '/');
                if (/\bdriver\s*\.\s*(findElement|click|sendKeys)\b/.test(content))
                    driverFiles.push(rel);
                if (/\bbrowser\s*\.\s*\$\b|\bawait\s+\$\s*\(/.test(content))
                    browserFiles.push(rel);
            }
            catch { /* skip */ }
        }
